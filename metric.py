...
def to_mask(pred, target, upsampling: int = None):
    upsampling = upsampling or 1
    ...
    for i, sample in enumerate(pred_boxes):
        pred_mask = Image.new('L', (pred[i]['masks'].size(-2)*upsampling, pred[i]['masks'].size(-1)*upsampling), color=0)
